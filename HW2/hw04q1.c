...
{	
	...
	schoolYear input; //for conversion
	...
	for (int i = 0; i < count - 1; i++){
		for (int j = 0; j < count - i - 1; j++){
			if (strcmpi(list[j].studentName, list[j + 1].studentName) > 0){
				studentTemp = list[j + 1];
				list[j + 1] = list[j];
				list[j] = studentTemp;
			}
		}
...
void display()
{
	...
	for (int i = 0; i < count; i++) { 
		...
		//start printing with proper lines
		printf("\n");
...
void load(char* fileName)
{
	...
	if (file)
	{
		...
		for (int i = 0; i < count; i++)
		{
			...
			fread(&list[i].schoolYear, sizeof(list[i].schoolYear), 1, file);
			fread(&list[i].studentID, sizeof(list[i].studentID), 1, file);
		}
		//after reading display success
		printf("Students record loaded from Student_List.txt.\n");
		fclose(file);
	}
	//file not there yet
	else
